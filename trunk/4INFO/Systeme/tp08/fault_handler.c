...
void unloadAPage(){
	srand(time(NULL));
	int ind = (int)(rand()%allocated_pages_size);
	if(mmu_isDirty(allocated_pages[ind])){
		swap_write(allocated_pages[ind],NULL);
	}
	allocated_pages_size--;
	mmu_invalidatePage(allocated_pages[ind]);
	allocated_pages[ind] = allocated_pages[allocated_pages_size+1];
	allocated_pages[allocated_pages_size+1] = NOTMAPPED;
}

void loadAPage(page_virt pv, page_phys pp){
	// Fill
	allocated_pages[allocated_pages_size] = ALLOCATED;
	allocated_pages_size++;
}
#endif
